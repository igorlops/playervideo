...
const VideoPlayer = () => {
  ...
  const totalTime = useMemo(() => {
    return TimeService.formatTime(video.duration)
  }, [video])
  ...
  useEffect(() => {
    ...
    if(isPlaying){
      progressTimer.current = setInterval(onProgress, 1)
    }
    return ()=> {
    ...
    <div className='video-player'>
      ...
      {video.url && (
        ...
        <div className="controls">
          ...
          <input type="range" min={0} onChange={onChangeProgress} value={progress} max={video.duration} step={.1}/>
        </div>
        <h2>{video.title}</h2>
        </>
      )}
